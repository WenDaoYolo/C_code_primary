...
/*
	...
	其他思路：编程中的递归来源于数学中的递归思维，因此用能数学公式表示出问题，那么递归编程就很容易去实现
...
/*
	...
	通过分析:从第三项开始,每一项都等于前两项的和,所以可以确定最简单基本的事件就是第一个元素0和第二个元素1		时间复杂度2^n,空间复杂度n
	这两个元素就可以用于控制递归的结束,后面的元素可以表示为F（n）=F(n-1)+F(n-2)		 两层递归，左边的递归完，加号右边的开始递归，最后均递归完得到结果返回
...
int main() {
	...
	//斐波那契前20项	     迭代思维				a=0	b=1 c=a+b....
	int a = 0, b = 1, c;
	printf("%d %d ",a,b);
	for (int i = 1; i <=20-2; i++) {	//每次更新迭代都可以求出一个数，已知两个数剩下n-2（18）个则需要进行n-2(18)次更新迭代
		c = a + b;
		printf("%d ", c);
			a = b;				//初始定义a为0，b=1，每次通过a和b求出第三项c,然后再更新迭代a和b
			b = c;                           //注意计算出的数字太大，变量会存不下发生溢出，则会返回错误数值
	}
